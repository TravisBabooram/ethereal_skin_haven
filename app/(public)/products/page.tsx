      ...
      <section style={{ paddingTop: 160, paddingBottom: 80, background: "var(--bg)", textAlign: "center", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ position: "relative", maxWidth: 700, margin: "0 auto", padding: "0 32px" }}>
          <AnimatedSection>
            ...
            <h1 style={{ fontFamily: "var(--font-cormorant, Georgia, serif)", fontSize: "clamp(48px, 7vw, 80px)", fontWeight: 300, color: "var(--text)", margin: "0 0 20px", lineHeight: 1.05 }}>
              Professional Products
            </h1>
            <p style={{ fontSize: 15, lineHeight: 1.75, color: "var(--text-muted)", maxWidth: 520, margin: "0 auto" }}>
      ...
      <section className="section-pad" style={{ background: "var(--bg)" }}>
        <div style={{ maxWidth: 1000, margin: "0 auto", padding: "0 32px" }}>
              ...
              {filtered.map((p, i) => (
                <AnimatedSection key={p.id} delay={i * 0.08}>
                  <article className="card-base" style={{ overflow: "hidden", display: "flex", flexDirection: "column" }}>
                    <div style={{ height: 260, background: "linear-gradient(135deg, var(--bg-elevated) 0%, rgba(201,169,110,0.1) 100%)", display: "flex", alignItems: "center", justifyContent: "center", position: "relative", overflow: "hidden" }}>
                      {p.image ? (
                      ...
                      <span style={{ position: "absolute", top: 16, right: 16, fontSize: 9, letterSpacing: "0.25em", color: "var(--gold)", textTransform: "uppercase", background: "var(--bg-card)", padding: "4px 12px", border: "1px solid var(--border)" }}>{p.category}</span>
                      {p.availabilityStatus !== "available" && (
                        <span style={{ position: "absolute", bottom: 16, left: 16, fontSize: 9, color: "#e05555", background: "var(--bg-card)", padding: "4px 10px", border: "1px solid rgba(220,60,60,0.3)", letterSpacing: "0.2em", textTransform: "uppercase" }}>Low Stock</span>
                      )}
                    </div>

                    <div style={{ padding: "28px 32px 32px", flex: 1, display: "flex", flexDirection: "column" }}>
                      <h3 style={{ fontFamily: "var(--font-cormorant, Georgia, serif)", fontSize: 22, fontWeight: 400, color: "var(--text)", margin: "0 0 12px", lineHeight: 1.3 }}>{p.name}</h3>
                      <p style={{ fontSize: 14, lineHeight: 1.75, color: "var(--text-muted)", margin: "0 0 20px", flex: 1 }}>{p.description}</p>
                      ...
                      <div style={{ borderTop: "1px solid var(--border)", paddingTop: 18, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                        <div>
                          <span style={{ fontFamily: "var(--font-cormorant, Georgia, serif)", fontSize: 26, color: "var(--gold)", fontWeight: 500 }}>${p.price}</span>
          ...
          <AnimatedSection delay={0.3}>
            <div style={{ marginTop: 64, padding: "32px 36px", background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 8 }}>
              ...
              <p style={{ fontSize: 14, color: "var(--text-muted)", margin: "0 0 12px", lineHeight: 1.7 }}>
                We use only professional-grade formulas in every treatment — including <strong style={{ color: "var(--text)" }}>Esthemax</strong>, <strong style={{ color: "var(--text)" }}>Starpil</strong>, <strong style={{ color: "var(--text)" }}>Bushbalm</strong>, and <strong style={{ color: "var(--text)" }}>Nova Wax</strong>. All tools are sanitised with <strong style={{ color: "var(--text)" }}>Barbicide</strong> between every client.
              </p>
